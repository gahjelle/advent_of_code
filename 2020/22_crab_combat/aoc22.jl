...
function solve(filename)
    ...
    # Read from file
    deck1, deck2 = open(filename) do fid
        @pipe fid |> readlines |> join(_, "\n") |> split(_, "\n\n") |> map(c -> parse_cards(c), _)
    end
    num_cards = length(deck1) + length(deck2)

    # Part 1
    regular_combat(deck1 |> copy, deck2 |> copy) |> score_deck(num_cards) |> println

    # Part 2
    @pipe recursive_combat(deck1 |> copy, deck2 |> copy) |> _.winning_deck |> score_deck(num_cards) |> println
end
