...
def read_starting_position(fid):
    re_generators = re.compile('a (\w+) generator')
    re_microchips = re.compile('a (\w+)-compatible microchip')
...
def remove_duplicates(positions):
    ...
    for position in positions:
        ...
        if not description in DUPLICATES:
...
def describe_position(position):
    ...
    for idx, floor in enumerate(position[1][::-1]):
        ...
        floor_txt.append('{}: {} {}'.format(floor_num, 'E' if position[0] == floor_num - 1 else '.',
                                            ' '.join(n if n in floor else '...' for n in all_names)))
    return '\n'.join(floor_txt)
