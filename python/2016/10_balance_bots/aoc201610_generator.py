...
def parse_data(puzzle_input):
    ...
    return distribute(bots, comparisons)
...
def distribute(bots, comparisons):
    ...
    >>> list(distribute({2: [9, 4], 1: [6]}, {2: (1, 0), 1: (1001, 0), 0: (1002, 1000)}))
    [(2, 4, 9), (1, 4, 6), (0, 6, 9), {1: 4, 2: 6, 0: 9}]
    """
    outputs = {}
    while comparisons:
        for bot, chips in bots.items():
            ...
            to_low, to_high = comparisons[bot]
            bots = {b: c for b, c in bots.items() if b != bot} | {
                b: bots.get(b, []) + [c]
                for b, c in [(to_low, low), (to_high, high)]
                if b < 1000
            }
            outputs = outputs | {
                o: c for o, c in [(to_low, low), (to_high, high)] if o >= 1000
            }
            comparisons = {b: c for b, c in comparisons.items() if b != bot}

    yield {output - 1000: value for output, value in outputs.items()}
